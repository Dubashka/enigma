...
def _render_step_upload() -> None:
    render_steps(current=1, steps=STEPS)
...
def _render_step_convert() -> None:
    render_steps(current=2, steps=STEPS)
    ...
    with col_convert:
        if st.button("Конвертировать", type="primary", use_container_width=True):
            ...
            with st.spinner("Конвертируем…"):
                md_text, error = _convert(file_path)
            if error:
                st.error(error)
            else:
...
def _render_step_result() -> None:
    render_steps(current=3, steps=STEPS)
    md_text: str = st.session_state[_MD_RESULT]
    file_name = st.session_state.get(_FILE_NAME, "файл")
...
def _count_pages(file_path: str) -> int | str:
    """Return page count using pymupdf if available, else '?'."""
    try:
        import fitz  # pymupdf
...
def _convert(file_path: str) -> tuple[str, str | None]:
    """Convert PDF to Markdown using markitdown. Returns (md_text, error_or_None)."""
    try:
...
def _cleanup() -> None:
    """Remove temp file and clear session state."""
